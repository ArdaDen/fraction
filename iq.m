%% 
% Although the code can find the delay very precisely, it is not sufficient 
% since it cannot solve the ambiguity in angle. For this purpose, the IQ data 
% is used.
%% IQ DATA
...
%     low_pass_filter = designfilt("lowpassfir","FilterOrder",128,"DesignMethod","maxflat","HalfPowerFrequency",fc*0.08,"SampleRate",Fs);
    low_pass_filter = fir1(32,200/Fs);
    % fvtool(low_pass_filter,"MagnitudeDisplay","magnitude")
    q_data_filtered = filter(low_pass_filter,1,q_data); % Sine theta
    i_data_filtered = filter(low_pass_filter,1,i_data); % Cosine theta
